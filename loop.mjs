...
import {
  ...
  minePyPI, mineCrates, mineHF, mineJsonResume, mineOrcid, AI_TOPICS, aiForkedRepos, mineForks,
...
function sourceScores() {
  ...
  return {
    ...
    graph: score("graph"),
    orgs: score("orgs"),
    registries: Math.max(score("pypi"), score("crates"), score("hf"), score("jsonresume"), score("orcid")),
  };
}
...
async function tick() {
  ...
  if (type === "orgs") {
    // Company org members — highest public-LinkedIn density (top-firm professionals).
    const orgs = orgPool(pool);
    const org = orgs[(orgPage) % orgs.length];
    const page = (Math.floor(orgPage++ / orgs.length) % 5) + 1;
    const rows = await mineOrg(org, page);
    stash("orgs", "orgs", rows);
    return `org ${org} p${page} +${rows.length}`;
  }
  if (type === "registries") {
    // Rotate ORCID / JSON-Resume / PyPI / crates / HuggingFace — structured + niche.
    const p = regPage++, which = p % 5;
    if (which === 0) {
      const rows = await mineOrcid(((Math.floor(p / 5)) % 250) * 100); // email+linkedin in data, no GitHub cost
      stash("registries", "orcid", rows);
      return `orcid +${rows.length}`;
    }
    if (which === 1) {
      const rows = await mineJsonResume((Math.floor(p / 5) % 30) + 1);
      stash("registries", "jsonresume", rows);
      return `jsonresume +${rows.length}`;
    }
    if (which === 2) {
      const rows = await minePyPI((Math.floor(p / 5) % 300) + 1);
      stash("registries", "pypi", rows);
      return `pypi +${rows.length}`;
    }
    if (which === 3) {
      const rows = await mineCrates((Math.floor(p / 5) % 40) + 1);
      stash("registries", "crates", rows);
      return `crates +${rows.length}`;
    }
    const rows = await mineHF((Math.floor(p / 5) % 20) + 1);
